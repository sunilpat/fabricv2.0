...
function main {
   ...
   ORDERER_PORT_ARGS="-o 192.168.1.205:7050 --tls true --cafile $CA_CHAINFILE"
   ...
   for ORG in $PEER_ORGS; do
      ...
      while [[ "$COUNT" -le $NUM_PEERS ]]; do
         export CORE_PEER_ADDRESS=192.168.1.205:$((PORT++))
   ...
   makePolicy
   initPeerVars org2 0
   export CORE_PEER_ADDRESS=192.168.1.205:7053
   switchToAdminIdentity
   ...
   initPeerVars org1 0
   export CORE_PEER_ADDRESS=192.168.1.205:7051 
   ...
   initPeerVars org2 1
   export CORE_PEER_ADDRESS=192.168.1.205:7054 
   installChaincode
