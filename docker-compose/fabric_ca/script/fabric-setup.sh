...
function enrollCAAdmin {
   ...
   fabric-ca-client enroll -d -u https://admin:adminpw@${HOST}
}
...
function enrollOrdererIdentities {
   ...
   for ORG in $ORDERER_ORGS; do
      ...
      fabric-ca-client getcacert -d -u https://$HOST -M $ORG_MSP_DIR
...
function generateChannelArtifacts() {
  ...
  configtxgen -configPath /data -channelID $CHANNEL_NAME -profile TwoOrgsOrdererGenesis -outputBlock $GENESIS_BLOCK_FILE
  if [ "$?" -ne 0 ]; then
...
cp /data/jq/jq-linux64 /usr/local/bin
